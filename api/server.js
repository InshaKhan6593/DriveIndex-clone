...
// ── OPS DASHBOARD ────────────────────────────────────────────────────────────────────────
// Everything under /api/admin is operational, not part of the DriveIndex product surface.

// Pipeline definition + live lock state. STAGES is read from jobs/cron.js rather than
// duplicated, so the dashboard can never drift from what the cron actually runs.
app.get("/api/admin/pipeline", (req, res) => {
  const lockPath = path.join(__dirname, "..", "data", "cron.lock");
  let lock = null;
  try {
    let raw = fs.readFileSync(lockPath, "utf8");
    if (raw.charCodeAt(0) === 0xfeff) raw = raw.slice(1); // PowerShell writes a BOM
    lock = JSON.parse(raw);
  } catch { /* absent = not locked */ }
  ...
  res.json({
    stages: STAGES.map((s) => ({ name: s.name, cmd: s.cmd.join(" "), budgetMinutes: s.budget / 60000, optional: s.optional })),
    lock: lock ? { ...lock, alive: lockAlive } : null,
  });
});

// Cron run history. jobs/cron.js appends one JSON object per line to data/cron-history.jsonl.
// The file legitimately may not exist yet — a pipeline that has never completed a run has no
// history, which is different from an error.
app.get("/api/admin/jobs", (req, res) => {
  const historyPath = path.join(__dirname, "..", "data", "cron-history.jsonl");
  let runs = [];
  try {
    runs = fs.readFileSync(historyPath, "utf8")
      .split("\n").filter(Boolean)
      .map((l) => { try { return JSON.parse(l); } catch { return null; } })
      .filter(Boolean)
      .reverse();
  } catch { /* no history yet */ }
  res.json({ runs: runs.slice(0, Number(req.query.limit) || 100), exists: runs.length > 0 });
});

// Per-source state: what each scraper has produced, and how fresh it is.
app.get("/api/admin/sources", (req, res) => {
  const sales = db.prepare(
    `SELECT source, COUNT(*) rows, MAX(sold_at) newest, MIN(sold_at) oldest,
            SUM(CASE WHEN mileage IS NOT NULL THEN 1 ELSE 0 END) withMileage,
            SUM(CASE WHEN vin IS NOT NULL AND vin != '' THEN 1 ELSE 0 END) withVin,
            SUM(CASE WHEN price_usd IS NOT NULL THEN 1 ELSE 0 END) withUsd
     FROM sale GROUP BY source ORDER BY rows DESC`
  ).all();
  const listings = db.prepare(
    `SELECT source, COUNT(*) rows, SUM(CASE WHEN is_active=1 THEN 1 ELSE 0 END) active
     FROM listing GROUP BY source ORDER BY rows DESC`
  ).all();
  const queued = db.prepare(
    "SELECT source, COUNT(*) n FROM car_resolution_queue WHERE status='pending' GROUP BY source"
  ).all().reduce((a, r) => ({ ...a, [r.source]: r.n }), {});

  res.json({
    sales: sales.map((s) => ({
      ...s,
      pctMileage: s.rows ? s.withMileage / s.rows : 0,
      pctVin: s.rows ? s.withVin / s.rows : 0,
      pctUsd: s.rows ? s.withUsd / s.rows : 0,
      queued: queued[s.source] ?? 0,
    })),
    listings,
  });
});

// Review queue, filterable. This is the surface that makes "never a silent guess" meaningful —
// a queue nobody can see is a queue nobody works.
app.get("/api/admin/queue", (req, res) => {
  const limit = Math.min(Number(req.query.limit) || 50, 500);
  const page = Math.max(Number(req.query.page) || 1, 1);
  const { reasonClass, source, q } = req.query;

  const clauses = ["status = 'pending'"];
  const params = [];
  if (reasonClass) { clauses.push("reason_class = ?"); params.push(reasonClass); }
  if (source) { clauses.push("source = ?"); params.push(source); }
  if (q) { clauses.push("lower(raw_title) LIKE lower(?)"); params.push(`%${q}%`); }
  const where = `WHERE ${clauses.join(" AND ")}`;

  const items = db.prepare(
    `SELECT id, source, source_lot_id, raw_title, extracted_year, extracted_make, extracted_model,
            reason, reason_class, best_candidate_score, created_at
     FROM car_resolution_queue ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
  ).all(...params, limit, (page - 1) * limit);

  const total = db.prepare(`SELECT COUNT(*) n FROM car_resolution_queue ${where}`).get(...params).n;
  const byClass = db.prepare(
    "SELECT reason_class, COUNT(*) n FROM car_resolution_queue WHERE status='pending' GROUP BY reason_class ORDER BY n DESC"
  ).all();
  const bySource = db.prepare(
    "SELECT source, COUNT(*) n FROM car_resolution_queue WHERE status='pending' GROUP BY source ORDER BY n DESC"
  ).all();
  const byStatus = db.prepare(
    "SELECT status, COUNT(*) n FROM car_resolution_queue GROUP BY status"
  ).all().reduce((a, r) => ({ ...a, [r.status]: r.n }), {});

  res.json({ page, limit, total, items, byClass, bySource, byStatus });
});
